...
## Симптом
Количество реально выполненных шагов симуляции не соответствует значению параметра `steps`.
Нумерация шагов начинается с 0, а последний шаг не выполняется.

## Как воспроизвести
Запустить симуляцию с любым сидом

## Отладка:
Установлен breakpoint на цикл `for`.
В конце отладки последнее значение шага на 1 меньше нужного.

## Причин
Неверно задан диапазон цикла — классическая off-by-one ошибка при использовании `range`.
...
## Проверка
Количество шагов и их нумерация полностью соответствуют параметру `steps`.
